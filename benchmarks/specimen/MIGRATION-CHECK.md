...
- new app, new schema (`sideworld-mc/specimen/conversations:scenario-b`) — the recreated service answered a round trip **470 s** after the swap: **14 passed, 18 failed of 33** — **0 new** failure(s) against the unmodified fork; 2 of the known failures passed this time; 18 known failures still failing; the suite's own words: 16× `POST /v1/conversations: expected 201, got 502`; 1× `POST /v1/tags: expected 201, got 502`
...
<sub>Sideworld Migration Check · run `20260923T221954Z` · fork restore 3.1 s / 3.1 s · the safe pattern's numbers are from run `20260923T172353Z`, same baseline, same workload; everything else is from this run on this box</sub>
