...
runtime built for the specimen (`../paraglobe/vm/`, `docs/FORK-EXPERIMENT-{1,2,3}.md`), with **zero changes to
...
| 9 | 06:12–06:28 | `../paraglobe/vm/mkfork-generic.{sh,py}`, TT fork map + runner; fork 1, fork 2, ceiling, unfork | 0.27 | fork-compose | |
...
| 14 | 07:02–07:12 | recovery evidence, teardown, host verification, specimen `../paraglobe/vm/boot.sh 1` proof, write-up | 0.17 | writeup | |
...
| `../paraglobe/vm/mkfork-generic.sh` + `../paraglobe/vm/mkfork-generic.py` | 63 + 36 | generic fork tool | `../paraglobe/mkfork.sh` generalised to any Compose project; specimen behaviour is the default |
...
| `../paraglobe/vm/build-rootfs-generic.sh` + `../paraglobe/vm/guest-generic/` (7 files) | 260 + 95 | generic rootfs builder | any Compose app: APP_DIR, COMPOSE_FILES, ENV_FILE, DATA_MAP; generic `app.service`, image loader, DB-aware quiesce/thaw/post-restore (Postgres CHECKPOINT / Mongo fsyncLock / MySQL flush) |
| `../paraglobe/vm/{boot,net,stop,bake-rootfs,snapshot,net-ns,fork,unfork}.sh` | +~60 lines of env knobs | parameters, specimen defaults | ROOTFS_IMG, SNAPSHOT, CLONE_PREFIX, SNAPFORK_PREFIX, FC_ID/FC_PREFIX, GUEST_HTTP, HEALTH_PATH, READY_FILE, GUEST_PORTS, APP_UNIT, APP_DIR_GUEST |
| `benchmarks/trainticket/vm-data.map`, `vm.sh` | 26 + 34 | TT wiring for the vm tooling | keeps every name under `../paraglobe/vm/out/tt-*`, `tank/tt-*`, `/run/fc-tt*` |
| `benchmarks/trainticket/vm-fork-measure.sh` | 48 | per-fork VM measurement | restore, PSS@10/60/120, isolation write, storage delta, host headroom → `../paraglobe/vm/out/tt-forks.csv` |
...
Tooling: `../paraglobe/vm/mkfork-generic.sh` + `../paraglobe/vm/mkfork-generic.py` (the specimen's `../paraglobe/mkfork.sh` generalised:
...
Tooling: `../paraglobe/vm/build-rootfs-generic.sh` + `../paraglobe/vm/guest-generic/` (new), the eight existing `../paraglobe/vm/` scripts
...
| | |
|---|---|
| rootfs build | `../paraglobe/vm/out/tt-rootfs.ext4`, 24 GiB image, **61 s**: 45 images → 2.2 GB archive; 2.9 GB used before load |
...
contacts, `3<kk>22` ssh. Series in `../paraglobe/vm/out/tt-forks.csv`.
...
- `../paraglobe/vm/out/tt-rootfs.ext4` (24 GiB sparse, 6 GB used), `../paraglobe/vm/out/tt-images.tar` (2.2 GB),
  `../paraglobe/vm/out/snap-ttbase/` (memory file 4.9 GB allocated + a reflinked rootfs copy), `../paraglobe/vm/out/tt-forks.csv`,
...
their snapshots, `../paraglobe/vm/out/tt-*`, `../paraglobe/vm/out/snap-ttbase/`, both `/tank/work/trainticket*` checkouts, the
...
(datasets, zvol, `../paraglobe/vm/out/tt-*`, snapshot dir, checkouts — **and the 45 images this time**), then
...
in `snapshot.sh` (no `head`), and `onboard.sh` now logs everything to `../paraglobe/vm/out/tt-onboard.log` and
