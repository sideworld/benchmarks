...
runtime built for the specimen (`vm/`, `docs/FORK-EXPERIMENT-{1,2,3}.md`), with **zero changes to
...
| `vm/mkfork-generic.sh` + `vm/mkfork-generic.py` | 63 + 36 | generic fork tool | `mkfork.sh` generalised to any Compose project; specimen behaviour is the default |
...
| `vm/build-rootfs-generic.sh` + `vm/guest-generic/` (7 files) | 260 + 95 | generic rootfs builder | any Compose app: APP_DIR, COMPOSE_FILES, ENV_FILE, DATA_MAP; generic `app.service`, image loader, DB-aware quiesce/thaw/post-restore (Postgres CHECKPOINT / Mongo fsyncLock / MySQL flush) |
...
Tooling: `vm/mkfork-generic.sh` + `vm/mkfork-generic.py` (the specimen's `mkfork.sh` generalised:
...
Tooling: `vm/build-rootfs-generic.sh` + `vm/guest-generic/` (new), the eight existing `vm/` scripts
...
| | |
|---|---|
| rootfs build | `vm/out/tt-rootfs.ext4`, 24 GiB image, **61 s**: 45 images → 2.2 GB archive; 2.9 GB used before load |
...
contacts, `3<kk>22` ssh. Series in `vm/out/tt-forks.csv`.
...
- `vm/out/tt-rootfs.ext4` (24 GiB sparse, 6 GB used), `vm/out/tt-images.tar` (2.2 GB),
  `vm/out/snap-ttbase/` (memory file 4.9 GB allocated + a reflinked rootfs copy), `vm/out/tt-forks.csv`,
