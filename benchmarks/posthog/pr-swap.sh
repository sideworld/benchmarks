...
# The "PR" is one line in posthog/views.py: /_health answers "ok sideworld-pr1" instead of "ok".
...
set -euo pipefail
HERE=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd); P=${PH_SRC:-/tank/work/posthog}; SW=${SIDEWORLD_DIR:-/tank/work/sideworld}
...
grep -q 'sideworld-pr1' "$F" || sed -i 's|        return HttpResponse("ok", status=status, content_type="text/plain")|        return HttpResponse("ok sideworld-pr1", status=status, content_type="text/plain")|' "$F"
grep -n 'sideworld-pr1' "$F" | sed 's/^/   /'
...
for _ in $(seq 1 2400); do v=$( { curl -s -m 5 "http://127.0.0.1:3${KK}80/_health" || true; } ); case "$v" in *sideworld-pr1*) break ;; esac; sleep 0.25; done; t4=$(date +%s.%N)
