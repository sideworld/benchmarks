...
| **PR → changed system serving** | — (not measured) | **9.0–10.8 s** (runs 1–2, by hand; 4.8 s of it inside the fork); 7.5 s scripted (`pr-swap.sh`, run 3) | **66.9 s** (`pr-swap.sh`, run 3): 1.7 s build + 2.6 s ship + 3.8 s load/swap, then **58.8 s of Rails booting** in the recreated container; siblings kept serving 4.7.2 |
| **PSS per idle fork** | **473 MB** @120 s (6 GiB guest; 449 MB in a 2 GiB guest); marginal fork ~400–500 MB from the third on; 2–5× once the fork does work | **3.0–3.6 GB** @120 s (24 GiB guest); ~5 GB after serving a search | **462 MB** median @120 s (8 GiB guest); 1,206 MB for the first fork alone, 395 MB by the tenth; ~245 MB private per fork; 564–999 MB after a post, its jobs and a timeline |
...
- **Customer-specific runtime code** — lines inside the fork runtime (`../micromonkis/vm/`, `mkfork*`) that name or
...
- **Generic tooling added** — lines added to the runtime while onboarding that system, from
  `git diff --numstat <commit before>..<commit after> -- vm ../micromonkis/mkfork.sh ../micromonkis/mkfork.py`; reusable by the next
...
Runtime: [`../micromonkis/vm/`](../micromonkis/vm/) (start at [`../micromonkis/vm/README.md`](../micromonkis/vm/README.md)) and `mkfork*.sh`
([`../micromonkis/mkfork.sh`](../micromonkis/mkfork.sh) for the specimen, [`../micromonkis/vm/mkfork-generic.sh`](../micromonkis/vm/mkfork-generic.sh) for any
...
[`benchmarks/trainticket/`](benchmarks/trainticket/). Mastodon: `../micromonkis/vm/onboard-generic.sh benchmarks/mastodon/app.spec`
...
`../micromonkis/vm/app.sh benchmarks/mastodon/app.spec restore-series mdbase 11 10`, `pr-swap.sh <k>`, `alt-baseline.sh up`;
