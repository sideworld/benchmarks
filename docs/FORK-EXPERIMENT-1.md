...
| 2. fork config | a generated per-fork Compose override (`../paraglobe/mkfork.sh` + `../paraglobe/mkfork.py`) that bind-mounts the clones, offsets every published port by N×1000, and renames the networks per project |
...
| 4. fork | `SNAP=baseline-10m-clean ./mkfork.sh 1` — `../paraglobe/mkfork.sh` now takes `SNAP=<snapshot>` (default `baseline-10m`) to select the clone origin — then `docker compose -p fork1 … up --wait` |
