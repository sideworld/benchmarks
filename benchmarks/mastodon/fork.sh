...
"$SIDEWORLD_DIR/vm/app-datasets.sh" "$HERE/app.spec" forkmap "tank/md-f{n}-" > "$HERE/fork.map"
t0=$(date +%s.%N)
FORK_MAP=$HERE/fork.map OFFSET=$OFF PROJECT=$P OUT=$OUT \
  "$SIDEWORLD_DIR/vm/mkfork-generic.sh" "$MD" "$N" "$SNAP" -f "$MD/docker-compose.yml" -f "$HERE/compose/docker-compose.md.yml" >/dev/null
