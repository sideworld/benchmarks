...
| **Cold ready** | native `compose up --wait` **31.9 s**; microVM cold boot **27.2 s** (baked rootfs, Phase 1 measurement in `../snowglobe/vm/README.md`; not re-measured) | native **81.9 s** (68/68 healthy, seeding included); microVM **96.4 s** all 68 healthy (UI answers at 16–22 s) | native **61.9 s** (`up --wait`, 7/7 healthy; 65.6 s to the full probe: streaming OK + a post processed by Sidekiq); microVM **64.3 s** all healthy (web `/health` 40.4 s), full probe confirmed inside the VM |
...
| **Customer-specific runtime code** | **0 lines** (the runtime's first builder, `../snowglobe/vm/build-rootfs.sh` + `../snowglobe/vm/guest/`, was written against it and later generalised) | **0 lines** — TrainTicket appears in the runtime in two comments. Adapters live outside the runtime: 704 lines in `benchmarks/trainticket/` plus a 727-line generated Compose override | **0 lines** — Mastodon appears in four comments (`app-snapshot-native.sh`, `app-datasets.sh`, `onboard-generic.sh`). Adapters beside it: 812 lines in 20 files (252 onboard, 225 generate, 335 measure) |
| **Generic tooling added** (lines, from git) | **2,781** lines in 33 files: `../snowglobe/vm/` + `mkfork.{sh,py}` as of `e5b854b`, the commit before TrainTicket | **+559 / −59** in the same paths (`91d9375^..HEAD`): `build-rootfs-generic.sh` 276, `mkfork-generic.{sh,py}` 102, `guest-generic/` 70, env knobs on 8 `../snowglobe/vm/` scripts | **+190 / −1** in `../snowglobe/vm/` (`bcc2f6b..HEAD`): the spec-driven runbook layer `onboard-generic.sh` 34, `app.sh` 27, `app-datasets.sh` 26, `app-snapshot-native.sh` 36, `app-zvol.sh` 24, `restore-series.sh` 19, `prefault.py` 13 + `PREFAULT=1` in `snapshot.sh` (+9), `unfork.sh` busy-retry (+2/−1); plus **58** lines of bulk-load profile in `benchmarks/lib/` (RI triggers off, FK validation after) that the specimen's and TrainTicket's generators inherit |
...
- **Customer-specific runtime code** — lines inside the fork runtime (`../snowglobe/vm/`, `mkfork*`) that name or
...
- **Generic tooling added** — lines added to the runtime while onboarding that system, from
  `git diff --numstat <commit before>..<commit after> -- vm ../snowglobe/mkfork.sh ../snowglobe/mkfork.py`; reusable by the next
...
Runtime: [`../snowglobe/vm/`](../snowglobe/vm/) (start at [`../snowglobe/vm/README.md`](../snowglobe/vm/README.md)) and `mkfork*.sh`
([`../snowglobe/mkfork.sh`](../snowglobe/mkfork.sh) for the specimen, [`../snowglobe/vm/mkfork-generic.sh`](../snowglobe/vm/mkfork-generic.sh) for any
...
[`benchmarks/trainticket/`](benchmarks/trainticket/). Mastodon: `../snowglobe/vm/onboard-generic.sh benchmarks/mastodon/app.spec`
...
`../snowglobe/vm/app.sh benchmarks/mastodon/app.spec restore-series mdbase 11 10`, `pr-swap.sh <k>`, `alt-baseline.sh up`;
