...
| 16 | 18:04–18:14 | PR swap (three attempts: env file, refused-connection poll, then timed), alternative baseline ×2, Compose re-measure, teardown of 10 forks + source, specimen `../paraglobe/vm/boot.sh 1` proof (27.2 s), KSM off | 0.17 | measurement | |
...
| `app.spec` | 25 | app spec | what `../paraglobe/vm/onboard-generic.sh` needs to know |
...
Generic, added to `../paraglobe/vm/` this time (the runbook layer TrainTicket showed was missing):

| file | lines | what |
|---|---|---|
| `../paraglobe/vm/onboard-generic.sh` | 40 | spec-driven runbook: clone → datasets → hooks → up → probe → generate → native snapshot → zvol → rootfs → bake → boot → VM snapshot |
| `../paraglobe/vm/app.sh` | 27 | spec-driven `vm.sh`: sets every knob from the spec; `VM_ENV_FILE` (guest-only env file, defaults to `ENV_FILE`) and `EXTRA_COPY` pass-through |
| `../paraglobe/vm/app-datasets.sh` | 27 | create/chown/snapshot datasets, emit `DATA_MAP` and `FORK_MAP` from the spec; a dataset two services share (web + sidekiq → `public/system`) is snapshotted once |
| `../paraglobe/vm/app-snapshot-native.sh` | 30 | quiesce by image (Postgres CHECKPOINT/VACUUM, Redis BGSAVE, Mongo, MySQL), clean stop, snapshot, start |
| `../paraglobe/vm/app-zvol.sh` | 20 | data zvol from dataset snapshots, per the map (shared dataset copied once) |
| `../paraglobe/vm/prefault.py` + `PREFAULT=1` in `snapshot.sh` | 14 + 8 | fault the memory file's allocated extents through a mapping after `/snapshot/create` (the TrainTicket requirement) |
| `../paraglobe/vm/guest-generic/…/quiesce` | +1 | Redis `BGSAVE` in the in-guest quiesce |
...
So the "generic onboarding" is, today, the generic *runtime* (`../paraglobe/vm/*`, `mkfork-generic`) plus a
...
`VACUUM=1 ../paraglobe/vm/app-snapshot-native.sh benchmarks/mastodon/app.spec md-base`: VACUUM ANALYZE +
...
`benchmarks/mastodon/fork.sh 1` (`../paraglobe/vm/mkfork-generic.sh` with the fork map from the spec, port
...
Rootfs: `../paraglobe/vm/app.sh … build` (generic builder, 6 images, `SIZE_MB=12288`) — 3 min the first
...
`../paraglobe/vm/app-zvol.sh` and `app-datasets.sh create` used `/<dataset-minus-tank>` (`/md-pg`) as the
...
**Boot 1** (17:27:52, `../paraglobe/vm/app.sh … boot 1`, 4 vCPU, **8,192 MiB**, `/dev/vdb` = clone of the zvol):
...
**Snapshot 1 → `mdbase`** (17:29:48, `PREFAULT=1 ../paraglobe/vm/app.sh … snapshot 1 mdbase`, 18.3 s wall):
...
**Series 2 — 10 forks of `mdbase`, 17:38:46–18:01 UTC** (`../paraglobe/vm/out/md-forks.csv`; each fork: restore, no requests for 120 s with PSS sampled at 10/60/120 s, then one status posted + Sidekiq checked + streaming health + storage delta):
...
**Warm restore series** (`../paraglobe/vm/restore-series.sh`, new and generic: fork slot 11 / measure / unfork, ten
...
`restore-series.sh` swallowed a failed unfork (now surfaced), and `../paraglobe/vm/unfork.sh` hit
...
`tank/md-vm-data@md-base` (60 GB); `../paraglobe/vm/out/md-rootfs.ext4` (12 GiB, baked), `../paraglobe/vm/out/snap-mdbase/`
(mem 8 GiB apparent, 1.5 GiB on disk — its ZFS side, `tank/md-vmfork1@mdbase`, went with the source
VM's clone at teardown, so a new fork series starts with `boot 1` + `snapshot 1`), `../paraglobe/vm/out/md-*.log`,
...
specimen project untouched and `../paraglobe/vm/boot.sh 1` re-verified at 27.2 s.
...
| | lines | files |
|---|---|---|
| **Generic, absorbed into `../paraglobe/vm/` this time** (from `git diff --numstat` and the new files) | **+190 / −1** — `app.sh` 27, `onboard-generic.sh` 34, `app-datasets.sh` 26, `app-snapshot-native.sh` 36, `app-zvol.sh` 24, `restore-series.sh` 19, `prefault.py` 13, `snapshot.sh` +9 (pre-fault), `unfork.sh` +2/−1 (busy retry) | 8 |
| **Generic, beside `../paraglobe/vm/`** — the bulk-load profile every generator inherits | **58** — `benchmarks/lib/BULK-LOAD.md` 23, `pg-bulk-load-begin.sql` 6, `pg-bulk-load-end.sql` 29; plus 5 lines added to `../specimen/data/scale/README.md` and 2 to TrainTicket's `gen.sh` | 3 + 2 |
