...
Branch: **main**. Tooling: `../snowglobe/vm/measure-mem.sh`.
...
Series in `../snowglobe/vm/out/measure-mem-<variant>.csv`, roll-up in `../snowglobe/vm/out/measure-mem-summary.csv`.
...
`../snowglobe/vm/ksm-exec` calls `prctl(PR_SET_MEMORY_MERGE, 1)` and `execv`s firecracker. The flag survives the
...
it replaces as `../snowglobe/vm/out/rootfs.prev.ext4`.
